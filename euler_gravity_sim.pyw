...
class Field:
  ...
  def mergePoints(p1, p2):
    ...
    return Point(
      (p1.xPos*pi.mass + p2.xPos*p2.mass)/cMass,
      (p1.yPos*pi.mass + p2.yPos*p2.mass)/cMass,
      (p1.xVel*pi.mass + p2.xVel*p2.mass)/cMass,
      (p1.yVel*pi.mass + p2.yVel*p2.mass)/cMass,
      cMass,
      p1.xScale,
      p1.yScale,
      p1.colour
      ...
      if pi != None:
        for j in range(len(self.pointArray)):
          ...
          if i == j or pj == None:
            ...
            r = ((pi.mass)**(1/3))/2000 + ((pj.mass)**(1/3))/2000
            if dist <= r:
              workArray[i] = mergePoints(pi, pj)
              workArray[j] = None
              ...
              workArray[i].xVel += Field.calcAccl(pi.mass, force, dist, pi.xPos, pj.xPos)
              workArray[i].yVel += Field.calcAccl(pi.mass, force, dist, pi.yPos, pj.yPos)
